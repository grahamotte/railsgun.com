  ...
  class DeploymentUser < Base
    class << self
      ...
      def apply
        ...
        Cmd.remote("sed -i -e '$a\\' ~/.ssh/authorized_keys")
        Cmd.remote('cat ~/.ssh/id_rsa.pub >> ~/.ssh/authorized_keys')
      end

      # ---

      def run_remote_root(cmd, *opts, bool: false)
        Cmd.call(cmd, user: 'root', host: Instance.ipv4, bool: bool)
      end
      ...
      def write_file_root(path, data)
        ...
        Cmd.local("scp -i #{Secrets.id_rsa_path} #{local_tmp_file} root@#{Instance.ipv4}:#{remote_tmp_file}")
